...
def main():
    parser = argparse.ArgumentParser(
        description='Distributed Experiment Runner Server')
    ...
    parser.add_argument('-rf', '--run-forever', default=False,
                        action='store_true', help='Disable auto termination of server')
    parser.add_argument('-mi', '--max-idle', default=3600, type=int,
                        help='Maximum idle time before auto termination (in seconds). Default 1 hour.')
    ...
    if args.log:
        logging_handlers.append(
            logging.FileHandler(filename=args.log, mode='w'))
