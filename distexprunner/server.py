...
class Server:
    def __init__(self, id: str, ip: str, port: int = 20000, **kwargs):
        ...
        for k, v in kwargs.items():
            if k.startswith('_'):
                raise Exception(
                    f'private/protected attributes not allowed, "{k}" starts with _ or __.')
    ...
    def send_file(self, source, target):
        ...
        with open(source, "rb") as source_file:
            ...
            task = self.__client.rpc.send_file(base64.b64encode(
                data).decode('utf-8'), target, checksum, mode=mode)
            loop.run_until_complete(task)
            logging.info(
                f'{self.id}: Send file source={source} target={target} checksum={checksum}')
    ...
    def run_cmd(self, cmd, stdout=None, stderr=None, stdin=None, env={}, timeout=None):
        ...
        rc_future, pid_future = loop.run_until_complete(task)
